...
def get_datasource_info(dataset_name):
    if dataset_name not in config.DATASOURCES:
        raise HTTPException(
            status_code=400, detail="Dataset {} not found".format(dataset_name)
        )
...
def get_datastore(dataset_name, mip):
    ...
    if datainfo["type"] == "neuroglancer_precomputed":
            ...
            with open(zattr_file) as f:
                ...
                if "voxel_offset" in zattr:
                    ...
                    for dim in range(len(zattr["voxel_offset"])):
                        outputmaps.append(
                            ts.OutputIndexMap(
                                offset=-zattr["voxel_offset"][dim], input_dimension=dim
                            )
                        )

                    if datainfo["width"] > 1:
                        outputmaps.append(
                            ts.OutputIndexMap(offset=0, input_dimension=len(outputmaps))
                        )
        ...
        raise HTTPException(
            status_code=400,
            detail="Datasource type '{}' not found".format(datainfo["type"]),
        )
